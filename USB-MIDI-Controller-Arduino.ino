...
// to avoid notes from lingering while it is shifting between "Special Mode"
void destroyNotes() {
  int i = 0;
...
void readButtons() {
    ...
    if ( ( curStateButtons[0] == 1 && curStateButtons[2] == 1 ) ) {
      ...
      if ( modeVal > 254 ) {
        ...
        if ( modeOn == 0 ) {
          destroyNotes();
          //Serial.println("Funky turned on!");
          modeOn = 1;
          analogWrite(ledM, 100);
          ledFunction();
        }
        else {
          //Serial.println("Funky turned off.");
          analogWrite(ledM, 0);
          analogWrite(ledL, 0);
          analogWrite(ledR, 0);
          modeOn = 0;
...
void readPots() {
  ...
  for ( i = 0; i < numPots; i++ ) {
    ...
    if ( lastValPots[i] != tmp ) {
      ...
      if ( diff <= -8 || diff >= 8 ) {                       // Avoidance of miniscule movement, will only change when it goes 6 up, to limit noise
        // Serial.println(tmp);
